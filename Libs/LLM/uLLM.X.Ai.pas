...
type
  EXAiError = class(ELLMException);

  TXGrokAI = class(TBaseLLM)
  protected
...
procedure TXGrokAI.CreateRESTClientAndRequest(out AClient: TRESTClient; out ARequest: TRESTRequest; out AResponse: TRESTResponse);
begin
  AClient := TRESTClient.Create(nil);
  ARequest := TRESTRequest.Create(nil);
  AResponse := TRESTResponse.Create(nil);

  AClient.BaseURL := 'https://api.x.ai';
...
procedure TXGrokAI.BuildJSONRequestBody(ARequest: TRESTRequest; ChatConfig: TChatSettings; AMessages: TObjectList<TChatMessage>);
var
...
begin
  ...
  if not LChoice.TryGetValue<TJSONObject>('message', LMessageJSON) then
    raise EXAiError.Create('Invalid response: missing message');
...
procedure TXGrokAI.HandleErrorResponse(AResponse: TRESTResponse);
var
  LJSONResponse: TJSONObject;
...
begin
  ...
  try
    ...
    begin
      raise EXAiError.CreateFmt(LJSONMsg.GetValue<string>('code'), AResponse.StatusCode, LJSONMsg.GetValue<string>('message'))
    end
    else
      raise EXAiError.CreateFmt('', AResponse.StatusCode, AResponse.StatusText);
  finally
    LJSONResponse.Free;
  end
  else
...
constructor TXGrokAI.Create(const APIKey: string);
begin
  inherited Create(APIKey);
  FFunctions := TFunctionRegistry.Create;
end;

destructor TXGrokAI.Destroy;
begin
  FreeAndNil(FFunctions);
  inherited;
end;

function TXGrokAI.Completion(const AQuestion: string; const AModel: string): string;
var
  LClient : TRESTClient;
...
begin
  ...
  try
    ...
    LClient.BaseURL := 'https://api.x.ai';
    ...
    begin
      ...
      raise EXAiError.Create('HTTP response code: ' + LResponse.StatusCode.ToString);
  finally
...
function TXGrokAI.GetModelInfo: TObjectList<TBaseModelInfo>;
var
...
procedure TXGrokAI.ListXAIModels(out AModelList: TStringList);
var
  LRESTClient: TRESTClient;
  LRESTRequest: TRESTRequest;
  LRESTResponse: TRESTResponse;
...
begin
  ...
  try
    LRESTClient := TRESTClient.Create('https://api.x.ai');

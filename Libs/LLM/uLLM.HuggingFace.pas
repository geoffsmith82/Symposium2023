...
type
  EHuggingFaceError = class(ELLMException);
...
begin
  ...
  if not LChoice.TryGetValue<TJSONObject>('message', LMessageJSON) then
    raise EHuggingFaceError.Create('Invalid response: missing message');
  Content := '';
...
begin
  ...
  try
    ...
    begin
      raise EHuggingFaceError.CreateFmt(LJSONMsg.GetValue<string>('type'), AResponse.StatusCode, LJSONMsg.GetValue<string>('message'))
    end
    else if LJSONResponse.TryGetValue<string>('error', error) then
    begin
      raise EHuggingFaceError.Create(error);
    end
    else
      raise EHuggingFaceError.CreateFmt('', AResponse.StatusCode, AResponse.StatusText);
  finally
    LJSONResponse.Free;
  end
  else

...
type
  EOpenRouterError = class(ELLMException);

  TOpenRouter = class(TBaseLLM)
  protected
...
function TOpenRouter.ChatCompletion(ChatConfig: TChatSettings; AMessages: TObjectList<TChatMessage>): TChatResponse;
var
...
procedure TOpenRouter.CreateRESTClientAndRequest(out AClient: TRESTClient; out ARequest: TRESTRequest; out AResponse: TRESTResponse);
begin
  AClient := TRESTClient.Create(nil);
  ARequest := TRESTRequest.Create(nil);
  AResponse := TRESTResponse.Create(nil);

  AClient.BaseURL := 'https://openrouter.ai';
  ...
  ARequest.Resource := '/api/v1/chat/completions';
...
procedure TOpenRouter.BuildJSONRequestBody(ARequest: TRESTRequest; ChatConfig: TChatSettings; AMessages: TObjectList<TChatMessage>);
var
...
class function TOpenRouter.SupportsStructuredOutput: Boolean;
begin
  Result := True;
end;

procedure TOpenRouter.ProcessResponse(LJSONResponse: TJSONObject; var AResponse: TChatResponse; out FunctionReturnValue: string; AMessages : TObjectList<TChatMessage>);
var
  LChoices: TJSONArray;
  LChoice: TJSONObject;
  LUsage: TJSONObject;
...
begin
  ...
  if not LChoice.TryGetValue<TJSONObject>('message', LMessageJSON) then
    raise EOpenRouterError.Create('Invalid response: missing message');
  Content := '';
...
procedure TOpenRouter.HandleErrorResponse(AResponse: TRESTResponse);
var
  LJSONResponse: TJSONObject;
...
begin
  ...
  try
    ...
    begin
      raise EOpenRouterError.CreateFmt(LJSONMsg.GetValue<string>('type'), AResponse.StatusCode, LJSONMsg.GetValue<string>('message'))
    end
    else
      raise EOpenRouterError.CreateFmt('', AResponse.StatusCode, AResponse.StatusText);
  finally
    LJSONResponse.Free;
  end
  else
...
constructor TOpenRouter.Create(const APIKey: string);
begin
  inherited Create(APIKey);
  FFunctions := TFunctionRegistry.Create;
end;

destructor TOpenRouter.Destroy;
begin
  FreeAndNil(FFunctions);
  inherited;
end;

function TOpenRouter.Completion(const AQuestion: string; const AModel: string): string;
var
  LClient : TRESTClient;
...
begin
  ...
  try
    ...
    LClient.BaseURL := 'https://openrouter.ai';
    LRequest.Resource := '/api/v1/completions';
    ...
    begin
      ...
      raise EOpenRouterError.Create('HTTP response code: ' + LResponse.StatusCode.ToString);
  finally
...
function TOpenRouter.GetModelInfo: TObjectList<TBaseModelInfo>;
var
...
procedure TOpenRouter.ListOpenAIModels(out AModelList: TStringList);
var
  LRESTClient: TRESTClient;
  LRESTRequest: TRESTRequest;
  LRESTResponse: TRESTResponse;
...
begin
  ...
  try
    LRESTClient := TRESTClient.Create('https://openrouter.ai');
    LRESTRequest := TRESTRequest.Create(nil);
    LRESTResponse := TRESTResponse.Create(nil);
    LRESTRequest.Client := LRESTClient;
    LRESTRequest.Resource := '/api/v1/models';
